...
export function Units() {
  ...
  const [selectedUnit, setSelectedUnit] = useState<UnitTableData>(
    {} as UnitTableData,
  )
  ...
  function handleChangeSelectedUnit(
  ...
  function onOpenModal(selectedId: number) {
    const selectedUnitData = tableData.filter(
        ...
        <Table.Column
          ...
          render={(_: any, record: UnitTableData) => (
            <div>
              ...
              <Confirm
                title={`Excluir unidade ${record.unit}`}
                onConfirm={() => deleteSelectedUnit(record)}
              />
            </div>
          )}
        />
